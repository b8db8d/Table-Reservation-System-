...
import { update as updateRoute, index as indexRoute } from '@/routes/admin/reservations';
...
const form = useForm({
    first_name: props.reservation.first_name,
    last_name: props.reservation.last_name,
    email: props.reservation.email,
    phone: props.reservation.phone,
    guest_count: props.reservation.guest_count,
...
function submit(): void {
    form.patch(updateRoute.url(props.reservation.id));
}
</script>

<template>
    <Head :title="`Edit ${reservation.reference_number}`" />

    <div class="flex flex-col gap-6 p-6">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-xl font-semibold">Edit Reservation</h1>
                <p class="mt-0.5 font-mono text-xs text-muted-foreground">{{ reservation.reference_number }}</p>
            </div>
        </div>
